#!/usr/bin/env python3
"""
Precisely replace the notebook container with catalogue view
"""
...
# Find the notebook container start and end
notebook_start = -1
notebook_end = -1
div_count = 0

for i, line in enumerate(lines):
    if 'class="notebook-container"' in line:
        notebook_start = i
        div_count = 1
    elif notebook_start != -1:
...
print(f"Found notebook container from line {notebook_start+1} to {notebook_end+1}")

if notebook_start != -1 and notebook_end != -1:
    # Create the catalogue content
    catalogue_lines = '''            <!-- Catalogue Container -->
...
'''
    ...
    # Replace the notebook container with catalogue
    new_lines = lines[:notebook_start] + [catalogue_lines] + lines[notebook_end+1:]
    
    # Write the updated content
    with open('/Users/daws/alphapulse/ui/research.html', 'w') as f:
        f.writelines(new_lines)
    
    print("✅ Successfully replaced notebook container with catalogue view")
    
    # Also need to add the JavaScript functions
    content = ''.join(new_lines)
    
    # Add the selectStrategy and searchStrategies functions
    js_functions = '''
        // Strategy selection
...
'''
    ...
    print("✅ Added JavaScript functions for strategy selection and search")
    
else:
    print("❌ Could not find notebook container boundaries")
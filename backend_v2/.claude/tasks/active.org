...
** TODO Hook Integration Implementation         :integration:hooks:automation:
   ...
   *** Acceptance Criteria (TDD Green Phase)
   ...
   - [ ] All tests pass
* TODO [#C] Mycelium Architecture Extraction     :architecture:refactor:library:
  :PROPERTIES:
  ...
  :DEADLINE:    <2025-10-01 Wed>
   ...
   :TRIGGER:     children todo!(NEXT)
  :END:
  Extract network module into standalone Mycelium library with three-layer architecture and monomorphic transport abstraction.
  
  **CRITICAL**: All agents working on this MUST read @.claude/docs/architecture/mycelium_separation.md
** TODO Mycelium Architecture Design Tests      :testing:tdd:architecture:
   :PROPERTIES:
   :ID:          MYCEL-EXTRACT-001-TESTS
   :EFFORT:      4h
   :ASSIGNED:    architecture-engineer
   :BRANCH:      test/mycelium-extraction
   :WORKTREE:    yes
   :TRIGGER:     ids(MYCEL-EXTRACT-001) todo!(NEXT)
   :TRIGGER:     ids(MYCEL-EXTRACT-001) todo!(NEXT)
   :END:
   Design tests for three-layer architecture separation.
   *** Acceptance Criteria (TDD Red Phase)
   - [ ] Test transport trait abstraction with multiple implementations
   - [ ] Test actor system with different transport backends
   - [ ] Test monomorphization (no vtable overhead)
   - [ ] Test local fast path (<100ns) vs remote path
   - [ ] Verify zero-cost abstractions via benchmarks
   - [ ] All tests initially fail (red phase)
** TODO Extract Mycelium Transport Layer        :transport:library:extraction:
   ...
   :EFFORT:      8h
   :ASSIGNED:    architecture-engineer
   :BRANCH:      feat/mycelium-transport
   :WORKTREE:    yes
   :BLOCKER:     ids(MYCEL-EXTRACT-001-TESTS)
   :BLOCKER:     ids(MYCEL-EXTRACT-001-TESTS)
   :END:
   Create mycelium-transport crate with pluggable backends.
   *** Acceptance Criteria (TDD Green Phase)
   - [ ] Transport trait with associated types (zero-cost)
   - [ ] TCP transport implementation
   - [ ] Unix socket transport implementation  
   - [ ] SharedMemory transport with local_deliver fast path
   - [ ] ZeroMQ transport implementation (optional)
   - [ ] All tests pass with <100ns local, <35μs IPC
** TODO Extract Mycelium Actor Runtime Tests    :testing:tdd:actors:
   :PROPERTIES:
   :ID:          MYCEL-EXTRACT-002-TESTS
   :EFFORT:      4h
   :ASSIGNED:    architecture-engineer
   :BRANCH:      test/mycelium-actors
   :WORKTREE:    yes
   ...
   :TRIGGER:     ids(MYCEL-EXTRACT-002) todo!(NEXT)
   :END:
   Design tests for actor runtime layer.
   *** Acceptance Criteria (TDD Red Phase)
   - [ ] Test actor lifecycle management
   - [ ] Test supervision trees and restart strategies
   - [ ] Test message routing with local vs remote actors
   - [ ] Test bundle optimization for cache locality
   - [ ] Benchmark actor message throughput >1M msg/s
   - [ ] All tests initially fail
** TODO Extract Mycelium Actor Runtime          :actors:runtime:library:
   :PROPERTIES:
   :ID:          MYCEL-EXTRACT-002
   :EFFORT:      12h
   :ASSIGNED:    architecture-engineer
   :BRANCH:      feat/mycelium-actors
   :WORKTREE:    yes
   :BLOCKER:     ids(MYCEL-EXTRACT-002-TESTS)
   :BLOCKER:     ids(MYCEL-EXTRACT-002-TESTS)
   :END:
   Create mycelium-actors crate with generic transport parameter.
   *** Acceptance Criteria (TDD Green Phase)
   - [ ] ActorSystem<T: Transport> with monomorphization
   - [ ] Actor trait with async message handlers
   - [ ] Bundle system for co-located actors
   - [ ] Supervision with exponential backoff

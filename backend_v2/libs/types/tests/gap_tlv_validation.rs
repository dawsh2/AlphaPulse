...
use alphapulse_types::protocol::tlv::{
    QuoteTLV, InvalidationReason, TradeTLV
};
use alphapulse_types::{InstrumentId, VenueId, RelayDomain, TLVType};
...
fn test_quote_tlv_type_assignment() {
    ...
    assert_eq!(
        TLVType::QuoteUpdate.relay_domain(),
...
fn test_quote_tlv_in_message_builder() {
    ...
    // Verify magic number
    assert_eq!(&message[0..4], &[0xDE, 0xAD, 0xBE, 0xEF]);
}
...
fn test_mixed_tlv_message() {
    ...
    // Verify both messages are valid Protocol V2
    assert_eq!(&trade_msg[0..4], &[0xDE, 0xAD, 0xBE, 0xEF]);
    assert_eq!(&quote_msg[0..4], &[0xDE, 0xAD, 0xBE, 0xEF]);
...
fn test_performance_characteristics() {
    ...
    assert!(ns_per_op < 10, "Serialization too slow: {}ns per op", ns_per_op);
...
fn test_state_management_integration() {
    ...
    let mut state_mgr = StateManager { invalidation_reason: None };
    assert!(state_mgr.is_valid());
    
    // Simulate disconnection
    state_mgr.invalidate(InvalidationReason::Disconnection);
    assert!(!state_mgr.is_valid());
    assert_eq!(state_mgr.invalidation_reason, Some(InvalidationReason::Disconnection));

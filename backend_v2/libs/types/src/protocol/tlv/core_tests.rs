...
#[cfg(test)]
mod tests {
    use crate::protocol::tlv::market_data::{TradeTLV, QuoteTLV, PoolSwapTLV};
    ...
    fn test_edge_cases_zero_values() {
        ...
        let trade = TradeTLV::new(
            VenueId::Ethereum,
            InstrumentId { venue: 1, asset_type: 1, reserved: 0, asset_id: 1 },
            0i64,              // zero price
            0i64,              // zero volume
            0u8,               // buy side
            0u64,              // zero timestamp
    ...
    fn test_edge_cases_max_values() {
        ...
        let trade = TradeTLV::new(
            VenueId::Ethereum,
            InstrumentId { venue: u16::MAX, asset_type: 255, reserved: 255, asset_id: u64::MAX },
            i64::MAX,          // max price
            i64::MAX,          // max volume
            u8::MAX,           // max side
            u64::MAX,          // max timestamp
    ...
    fn test_performance_requirement_under_1_second() {
        ...
        for i in 0..1000 {
            ...
            // Create PoolSwapTLV
            let swap = PoolSwapTLV::new(
                [i as u8; 20], [(i + 1) as u8; 20], [(i + 2) as u8; 20], 
                VenueId::Ethereum,
            ...
            let signal = ArbitrageSignalTLV::new(
                [i as u8; 20], [(i+1) as u8; 20], 
                300u16, 301u16,
                [i as u8; 20], [(i+2) as u8; 20],
                (i as f64) * 0.01, 1000.0, 150u16,
                5.0, 2.0, 1.0, 1640995200000000000u64 + i,
            );
    ...
    fn test_field_boundary_values() {
        ...
        for side in [0u8, 1u8, 127u8, 255u8] {
            let trade = TradeTLV::new(
                ...
                100000000i64, 50000000000i64, side, 1234567890000000000u64,
            );
        ...
        for spread in [0u16, 1u16, 100u16, 10000u16, u16::MAX] {
            let signal = ArbitrageSignalTLV::new(
                [0x11u8; 20], [0x22u8; 20], 300u16, 301u16,
                [0x33u8; 20], [0x44u8; 20], 10.0f64, 1000.0f64,
                spread, 5.0f64, 2.0f64, 1.0f64, 1640995200000000000u64,
            );
            test_roundtrip(signal, ARBITRAGE_SIGNAL_TLV_SIZE, &format!("ArbitrageSignalTLV spread_bps={}", spread));
        }
    ...
    fn test_padding_bytes_initialization() {
        ...
        let trade = TradeTLV::new(
            VenueId::Ethereum,
            InstrumentId { venue: 1, asset_type: 1, reserved: 0, asset_id: 123 },
            100000000i64, 50000000000i64, 1u8, 1234567890000000000u64,
        );

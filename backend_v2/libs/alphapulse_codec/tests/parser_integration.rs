...
use alphapulse_codec::{
    TLVMessageBuilder, parse_header, parse_tlv_extensions, find_tlv_by_type,
    extract_tlv_payload, validate_tlv_size, TLVType, ProtocolError
};
use alphapulse_types::{RelayDomain, SourceType, MESSAGE_MAGIC};
...
fn test_complete_message_parse_workflow() {
    ...
    // Verify TLV content
    match &extensions[0] {
        alphapulse_codec::TLVExtensionEnum::Standard(tlv) => {
            assert_eq!(tlv.header.tlv_type, TLVType::Trade as u8);
            assert_eq!(tlv.header.tlv_length, 40);
            assert_eq!(tlv.payload.len(), 40);
            
            // Verify payload data
            let parsed_trade = TestTradeTLV::read_from(&tlv.payload)
                .expect("Failed to parse trade data");
...
fn test_extended_tlv_parsing() {
    ...
    match &extensions[0] {
        alphapulse_codec::TLVExtensionEnum::Extended(tlv) => {
...
fn test_tlv_lookup_by_type() {
    ...
    let trade_payload = find_tlv_by_type(tlv_payload, TLVType::Trade as u8)
        .expect("Failed to find Trade TLV");
    assert_eq!(trade_payload, vec![0x01u8; 40]);
    
    let quote_payload = find_tlv_by_type(tlv_payload, TLVType::Quote as u8)
        .expect("Failed to find Quote TLV");
...
fn test_type_safe_payload_extraction() {
    ...
    let no_quote: Option<TestTradeTLV> = extract_tlv_payload(tlv_payload, TLVType::Quote)
        .expect("Failed to extract quote payload");
...
fn test_parsing_error_conditions() {
    ...
    let truncated_tlv = vec![
        ...
        0x01, 0x02,          // But only has 2 bytes
    ];
...
fn test_boundary_parsing() {
    ...
    let extended_message = TLVMessageBuilder::new(RelayDomain::Signal, SourceType::ArbitrageStrategy)
    ...
    let ext_extensions = parse_tlv_extensions(ext_tlv_payload).expect("Failed to parse extended boundary TLV");
...
fn test_mixed_format_parsing() {
    ...
    let message = builder.build().expect("Failed to build mixed format message");
    let tlv_payload = &message[MessageHeader::SIZE..];

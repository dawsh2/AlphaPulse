...
use alphapulse_codec::{TLVMessageBuilder, VendorTLVBuilder, TLVType};
...
fn test_round_trip_precision_preservation() {
    ...
    let reconstructed = TestTradeTLV::read_from(payload_bytes)
...
fn test_extreme_values() {
    ...
    let reconstructed = TestTradeTLV::read_from(payload_bytes)
        .expect("Failed to reconstruct max values");
    ...
    let zero_message = TLVMessageBuilder::new(RelayDomain::MarketData, SourceType::BinanceCollector)
    ...
    let zero_reconstructed = TestTradeTLV::read_from(zero_payload_bytes)
        .expect("Failed to reconstruct zero values");
...
fn test_buffer_operations() {
    ...
    let size = builder.build_into_buffer(&mut buffer)
...
fn test_build_and_send_workflow() {
    ...
    let result = builder.build_and_send(send_fn)
...
fn test_performance_with_many_tlvs() {
    ...
    let message = builder.build().expect("Failed to build message with many TLVs");

//! # Enhanced Validation System with All Critical Fixes
//!
...
//! - Zero-allocation parsing in hot path
//! - Pool discovery queue for unknown pools
...
/// Enhanced validation error types with more context
#[derive(Debug, Error)]
pub enum EnhancedValidationError {
    #[error("Protocol error: {0}")]
    Protocol(#[from] ProtocolError),
    
    #[error("Checksum mismatch on {data_type}: expected {expected:08x}, calculated {calculated:08x}")]
    ChecksumMismatch {
        expected: u32,
        calculated: u32,
        data_type: String,
    ...
    #[error("Message too large: {size} bytes > {max_size} limit for domain {domain:?}")]
    MessageTooLarge {
        size: usize,
        max_size: usize,
        domain: RelayDomain,
...
pub struct SequenceTracker {
    ...
    /// Recent sequences for duplicate detection (source -> set of recent sequences)
    ...
    pub fn validate_sequence(
        ...
        if let Some(&last_seq) = self.sequences.get(&source) {
            if sequence <= last_seq {
                // Sequence going backward or duplicate
                return Err(EnhancedValidationError::DuplicateSequence { source_id: source, sequence });
            }
            
            let gap = sequence - last_seq - 1;
            if gap > max_gap {
                return Err(EnhancedValidationError::SequenceGap {
                    source_id: source,
                    expected: last_seq + 1,
                    actual: sequence,
                    gap,
        ...
        if recent.len() > self.max_tracked {
            // Remove oldest (this is approximate, HashSet doesn't maintain order)
...
impl PoolDiscoveryQueue {
    ...
    pub fn queue_pool(&self, pool: [u8; 20]) -> Result<(), EnhancedValidationError> {
        ...
        Err(EnhancedValidationError::UnknownPool { pool })
    }
}

/// Enhanced TLV validator with all fixes
pub struct EnhancedTLVValidator {
...
impl EnhancedTLVValidator {
    /// Create new enhanced validator with configuration
    ...
    /// Create with pool discovery queue
    pub fn with_pool_discovery(mut self, queue: PoolDiscoveryQueue) -> Self {
        self.pool_discovery = Some(queue);
        self
    }

    /// Validate complete message with all checks
    pub fn validate_message<'a>(
        &self,
        full_message: &'a [u8],  // Full message including header for checksum
    ) -> Result<ValidatedMessage<'a>, EnhancedValidationError> {
        // Parse header
        let header = parse_header_safe(full_message)?;
        
        // Validate checksum on full message per Protocol V2 spec
        ...
        let relay_domain = RelayDomain::try_from(header.relay_domain)
            .map_err(|_| EnhancedValidationError::Protocol(
                ProtocolError::message_too_small(0, 0, "Invalid relay domain")
            ))?;
        
        // Validate message size
        self.validate_message_size(payload.len(), relay_domain)?;
        
        // Parse TLV extensions without allocation (using references)
        let tlv_extensions = self.parse_tlv_zero_copy(payload, relay_domain)?;
        
        Ok(ValidatedMessage {
            header: *header,
            tlv_extensions,
            validation_policy: format!("{:?}", relay_domain),
        })
    }
    ...
    fn validate_checksum(
        ...
        // Protocol V2 spec: checksum is calculated over entire message
        ...
        if header.checksum != 0 && header.checksum != calculated {
            return Err(EnhancedValidationError::ChecksumMismatch {
                expected: header.checksum,
                calculated,
                data_type: "full message per Protocol V2".to_string(),
            });
        }
    ...
    fn calculate_message_checksum(&self, full_message: &[u8]) -> u32 {
        ...
        hasher.update(&full_message[..CHECKSUM_OFFSET]); // Before checksum
        hasher.update(&full_message[CHECKSUM_OFFSET + 4..HEADER_SIZE]); // After checksum  
        hasher.update(&full_message[HEADER_SIZE..]); // Payload
        hasher.finalize()
    }

    /// Validate timestamp is within acceptable bounds
    pub fn validate_timestamp(&self, timestamp_ns: u64) -> Result<(), EnhancedValidationError> {
        if !self.config.timestamp.enforce_validation {
            return Ok(());
        }
        ...
        if timestamp_ns > max_future {
            return Err(EnhancedValidationError::InvalidTimestamp {
    ...
    /// Validate sequence number
    fn validate_sequence(&self, source: u8, sequence: u64) -> Result<(), EnhancedValidationError> {
    ...
    /// Validate message size with configurable limits
    fn validate_message_size(&self, size: usize, domain: RelayDomain) -> Result<(), EnhancedValidationError> {
        ...
        if size > max_size {
            return Err(EnhancedValidationError::MessageTooLarge {
                size,
                max_size,
                domain,
            });
    ...
    pub fn parse_tlv_zero_copy<'a>(
        ...
        payload: &'a [u8],
        domain: RelayDomain,
    ) -> Result<Vec<TLVExtensionZeroCopy<'a>>, EnhancedValidationError> {
        let mut extensions = Vec::new();
        let mut offset = 0;
        
        while offset < payload.len() {
            if offset + 2 > payload.len() {
                return Err(EnhancedValidationError::Protocol(
            ...
            if !self.is_valid_tlv_for_domain(tlv_type, domain) {
                return Err(EnhancedValidationError::InvalidTLVForDomain {
                    tlv_type,
                    domain,
                });
            }
            ...
            if tlv_type == 255 {
                ...
                // Zero-copy reference to payload
        ...
        // Check for pool addresses if DEX-related TLVs
        self.check_pool_addresses(&extensions)?;
        
        Ok(extensions)
    }

    /// Check for unknown pool addresses and queue for discovery
    fn check_pool_addresses(&self, tlvs: &[TLVExtensionZeroCopy]) -> Result<(), EnhancedValidationError> {
        ...
        for tlv in tlvs {
            ...
            // Check if this is a pool-related TLV (PoolSwap, PoolMint, etc.)
            if matches!(tlv_type, 11 | 12 | 13) && payload.len() >= 20 {
                // Extract pool address (first 20 bytes typically)
                ...
                if !known_pools.contains(&pool_addr) {
                    drop(known_pools); // Release read lock
                    
                    // Queue for discovery if we have a queue
    ...
    fn is_valid_tlv_for_domain(&self, tlv_type: u8, domain: RelayDomain) -> bool {
        match domain {
            RelayDomain::MarketData => (1..=19).contains(&tlv_type),
            RelayDomain::Signal => (20..=39).contains(&tlv_type),
            RelayDomain::Execution => (40..=79).contains(&tlv_type),
            _ => (100..=119).contains(&tlv_type), // System
        }
    }
...
/// Convert zero-copy extension to owned for compatibility
...
/// Validated message with all checks passed
#[derive(Debug)]
...
/// Safe header parsing with proper error handling
fn parse_header_safe(data: &[u8]) -> Result<&MessageHeader, EnhancedValidationError> {
    use crate::parser::parse_header;
    parse_header(data).map_err(|e| EnhancedValidationError::Protocol(e))
}
...
mod tests {
    ...
    fn test_sequence_validation() {
        ...
        // Backward sequence
        assert!(tracker.validate_sequence(1, 103, 10).is_err());
    }
    ...
    fn test_timestamp_validation() {
        ...
        // 30 seconds old should be ok
        assert!(validator.validate_timestamp(current_ns - 30_000_000_000).is_ok());
        
        // 2 minutes old should fail
        assert!(validator.validate_timestamp(current_ns - 120_000_000_000).is_err());
    }
    ...
    fn test_checksum_calculation() {
        ...
        message[0..4].copy_from_slice(&0xDEADBEEFu32.to_le_bytes()); // Magic
        
        let checksum = validator.calculate_message_checksum(&message);
        assert!(checksum != 0);
        
        // Modify message should change checksum
        message[40] = 0xFF;
        let checksum2 = validator.calculate_message_checksum(&message);

...
impl ServiceType {
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            "market_data_relay" => Some(ServiceType::MarketDataRelay),
            "signal_relay" => Some(ServiceType::SignalRelay),
            "execution_relay" => Some(ServiceType::ExecutionRelay),
            "polygon_publisher" => Some(ServiceType::PolygonPublisher),
            "flash_arbitrage" => Some(ServiceType::FlashArbitrage),
            "dashboard_websocket" => Some(ServiceType::DashboardWebSocket),
            "kraken_collector" => Some(ServiceType::KrakenCollector),
            "binance_collector" => Some(ServiceType::BinanceCollector),
            "coinbase_collector" => Some(ServiceType::CoinbaseCollector),
            _ => None,
        }
    }
}
...
impl ServiceEndpoint {
    ...
    pub async fn connect(&self) -> Result<UnixStream> {
        debug!("Connecting to service: {} at {}", self.service_type.name(), self.socket_path);
        
        let stream = UnixStream::connect(&self.socket_path)
            .await
            .context(format!("Failed to connect to {}", self.socket_path))?;
        
        debug!("Successfully connected to service: {}", self.service_type.name());
        Ok(stream)
    }
    ...
    pub async fn check_health(&mut self) -> bool {
        ...
        let is_healthy = match connection_result {
            Ok(Ok(_stream)) => {
                debug!("Health check passed for service: {}", self.service_type.name());
                true
            }
            Ok(Err(e)) => {
                debug!("Health check failed for service {}: {}", self.service_type.name(), e);
                false
            }
            Err(_) => {
                debug!("Health check timeout for service: {}", self.service_type.name());
                false
            }
        };
...
pub struct ServiceDiscovery {
    ...
    endpoints: Arc<RwLock<HashMap<ServiceType, Vec<ServiceEndpoint>>>>,
    /// Load balancing strategy
    load_balancing: LoadBalancingStrategy,
    /// Round-robin counter for load balancing
    round_robin_counter: Arc<RwLock<HashMap<ServiceType, usize>>>,
}
...
impl ServiceDiscovery {
    ...
    async fn load_config(environment: &Environment) -> Result<EnvironmentConfig> {
        ...
        let config_path = PathBuf::from("config/environments")
            .join(environment.config_file());
        ...
        warn!("Configuration file not found, using defaults for {:?}", environment);
    ...
    pub async fn resolve_service(&self, service_type: &ServiceType) -> Result<ServiceEndpoint> {
        ...
        let service_endpoints = endpoints.get(service_type)
            .ok_or_else(|| anyhow::anyhow!("No endpoints found for service: {:?}", service_type))?;

        if service_endpoints.is_empty() {
            return Err(anyhow::anyhow!("No endpoints configured for service: {:?}", service_type));
        }
        ...
        debug!("Resolved service {} to {}", service_type.name(), selected_endpoint.socket_path);
...
mod tests {
    ...
    fn test_environment_detection() {
        ...
        assert!(matches!(env, Environment::Development | Environment::Production | Environment::Docker | Environment::Testing));
    }

    #[test]
    fn test_service_type_parsing() {
        assert_eq!(ServiceType::from_str("market_data_relay"), Some(ServiceType::MarketDataRelay));
        assert_eq!(ServiceType::from_str("invalid_service"), None);
    }
    ...
    async fn test_service_endpoint_creation() {
        ...
        ).with_health_port(8001).with_priority(50);

...
use alphapulse_message_sink::{
    LazyMessageSink, LazyConfig, LazyConnectionState, Message, MessageSink, SinkError,
    test_utils::{CollectorSink, FailingSink, SlowSink, DelayedConnectionSink}
};
        ...
        move || {
            ...
            async move {
                if attempts < 2 {
                    ...
                    Err(SinkError::connection_failed(format!("Attempt {} failed", attempts + 1)))
                } else {
...
async fn test_metadata_and_health() {
    ...
    assert!(ext_metadata.capabilities.contains(&"lazy_connection".to_string()));
    assert!(ext_metadata.capabilities.contains(&"auto_reconnect".to_string()));

...
async fn test_composite_sink_creation_all_patterns() {
    ...
    let rr_sink = factory
        .create_sink("test_composite_round_robin")
        .await
        .unwrap();
    ...
    let failover_sink = factory
        .create_sink("test_composite_failover")
        .await
        .unwrap();
...
async fn test_factory_stats_accuracy() {
    ...
    let hit_rate =
        after_creation_stats.cached_sinks as f64 / after_creation_stats.total_services as f64;

...
impl CompositeSink {
    ...
    async fn send_fanout(&self, message: Message) -> Result<(), SinkError> {
        ...
        let send_futures: Vec<_> = self.targets
            .iter()
        ...
        if successful_sends == 0 {
            ...
            let errors: Vec<String> = results
                ...
                .collect();
            Err(SinkError::send_failed(format!("Fanout failed on all targets: {}", errors.join(", "))))
            ...
            let failed_targets: Vec<String> = results
                ...
                .filter_map(|(i, r)| r.as_ref().err().map(|e| format!("target[{}]: {}", i, e)))
                .collect();
            tracing::warn!("Fanout partial failure on {}/{} targets: {}", 
                         self.targets.len() - successful_sends, self.targets.len(),
                         failed_targets.join(", "));
    ...
    async fn send_round_robin(&self, message: Message) -> Result<(), SinkError> {
        ...
        match target.send(message).await {
            ...
            Err(e) => {
                self.messages_failed.fetch_add(1, Ordering::Relaxed);
                Err(SinkError::send_failed(format!("Round-robin target[{}] failed: {}", index, e)))
            }
        }
    }
    ...
    pub fn composite_metrics(&self) -> CompositeMetrics {
        CompositeMetrics {
            ...
            current_round_robin_index: self.round_robin_index.load(Ordering::Relaxed) % self.targets.len(),
        }
    }
}
...
impl MessageSink for CompositeSink {
        ...
        match self.pattern {
            CompositePattern::Fanout => {
                ...
                if connected_count == self.targets.len() {
                    ...
                    tracing::warn!("Fanout partially connected: {}/{} targets", connected_count, self.targets.len());
                    Ok(()) // Partial connection is acceptable
                } else {
                    Err(SinkError::connection_failed(format!("No targets connected: {}", errors.join(", "))))
                }
            }
    ...
    async fn disconnect(&self) -> Result<(), SinkError> {
        ...
        if errors.is_empty() {
            ...
            Err(SinkError::send_failed(format!("Disconnect errors: {}", errors.join(", "))))
        }
    }
    ...
    fn connection_health(&self) -> ConnectionHealth {
        let connected_targets = self.targets.iter().filter(|t| t.is_connected()).count();
        let healthy_targets = self.targets
            .iter()
    ...
    fn supports_multiplexing(&self) -> bool {
        ...
        self.targets.iter().all(|target| target.supports_multiplexing())
    }
}

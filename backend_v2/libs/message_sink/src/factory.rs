...
use crate::{
    config::{ServiceConfig, SinkType, CompositePattern, PrecisionContext},
    registry::ServiceRegistry,
    sinks::{CompositeSink, DirectSink, RelaySink},
    LazyMessageSink, LazyConfig, MessageSink, SinkError, MessageDomain
};
use std::sync::Arc;
...
impl SinkFactory {
    ...
    pub async fn create_sink(&self, service_name: &str) -> Result<Arc<dyn MessageSink>, SinkError> {
        ...
        let config = self.registry.lookup(service_name)
            .ok_or_else(|| SinkError::invalid_config(
        ...
        tracing::info!("Created sink for service '{}' (type: {:?})", service_name, config.sink_type);
        Ok(sink)
    }
    ...
    async fn create_relay_sink(
        ...
        let endpoint = config.endpoint.as_ref()
            .ok_or_else(|| SinkError::invalid_config(
                format!("Relay sink '{}' missing endpoint", service_name)
            ))?;
    ...
    async fn create_direct_sink(
        ...
        let endpoint = config.endpoint.as_ref()
            .ok_or_else(|| SinkError::invalid_config(
                format!("Direct sink '{}' missing endpoint", service_name)
            ))?;
        ...
        let sink = if endpoint.starts_with("tcp://") {
            ...
            return Err(SinkError::invalid_config(
                format!("Direct sink '{}' has invalid endpoint format: {}", service_name, endpoint)
            ));
        };
                ...
                move || {
                    ...
                    async move {
                        if endpoint.starts_with("tcp://") {
                            ...
                            Err(SinkError::invalid_config(
                                format!("Invalid endpoint format: {}", endpoint)
                            ))
                        }
                    }
                },
    ...
    async fn create_composite_sink(
        ...
        let pattern = config.pattern.ok_or_else(|| SinkError::invalid_config(
            format!("Composite sink '{}' missing pattern", service_name)
        ))?;
        
        let target_names = config.targets.as_ref()
            .ok_or_else(|| SinkError::invalid_config(
                format!("Composite sink '{}' missing targets", service_name)
            ))?;
        
        if target_names.is_empty() {
            return Err(SinkError::invalid_config(
                format!("Composite sink '{}' has empty targets list", service_name)
            ));
        }
        ...
        for target_name in target_names {
            ...
            if target_name == service_name {
                return Err(SinkError::invalid_config(
                    format!("Composite sink '{}' cannot target itself", service_name)
                ));
            }
            
            let target_sink = Box::pin(self.create_sink(target_name)).await
                .map_err(|e| SinkError::invalid_config(
                    format!("Failed to create target '{}' for composite sink '{}': {}", 
    ...
    pub async fn remove_from_cache(&self, service_name: &str) -> bool {
        ...
        if removed {
            tracing::info!("Removed sink '{}' from factory '{}' cache", service_name, self.name);
        }
        removed
    }
    ...
    pub fn validate_config(&self, service_name: &str) -> Result<(), SinkError> {
        let config = self.registry.lookup(service_name)
            .ok_or_else(|| SinkError::invalid_config(
                format!("Service '{}' not found in configuration. Available services: [{}]", 
                    service_name,
                    self.registry.service_names().map(|s| s.as_str()).collect::<Vec<_>>().join(", ")
                )
            ))?;
    ...
    fn validate_tlv_domain(&self, service_name: &str, domain: MessageDomain, sink_type: &SinkType) -> Result<(), SinkError> {
        match (domain, sink_type) {
            (MessageDomain::MarketData, SinkType::Relay) => {
                // Market data relay should use TLV types 1-19
                tracing::debug!("Service '{}' configured for Market Data domain (TLV types 1-19)", service_name);
            },
            (MessageDomain::Signals, SinkType::Relay) => {
                // Signal relay should use TLV types 20-39  
                tracing::debug!("Service '{}' configured for Signals domain (TLV types 20-39)", service_name);
            },
            (MessageDomain::Execution, SinkType::Relay) => {
                // Execution relay should use TLV types 40-79
                tracing::debug!("Service '{}' configured for Execution domain (TLV types 40-79)", service_name);
            },
            (domain, SinkType::Direct) => {
                tracing::debug!("Direct sink '{}' configured for {:?} domain", service_name, domain);
            },
            (domain, SinkType::Composite) => {
                tracing::debug!("Composite sink '{}' configured for {:?} domain", service_name, domain);
            },
    ...
    fn validate_precision_context(&self, service_name: &str, precision: PrecisionContext, sink_type: &SinkType) -> Result<(), SinkError> {
        match (precision, sink_type) {
            ...
            (PrecisionContext::TraditionalExchange, SinkType::Direct) => {
                tracing::debug!("Service '{}' will use 8-decimal fixed-point for USD prices", service_name);
            },
            (PrecisionContext::Mixed, _) => {
                tracing::debug!("Service '{}' will handle mixed precision with automatic detection", service_name);
            },
            (context, sink_type) => {
                tracing::debug!("Service '{}' ({:?}) configured with {:?} precision context", 
                    service_name, sink_type, context);
            }
        }
    ...
    fn validate_endpoint_accessibility(&self, service_name: &str, endpoint: &str) -> Result<(), SinkError> {
    ...
    fn validate_unix_socket_path(&self, service_name: &str, socket_path: &str) -> Result<(), SinkError> {
        ...
        if let Some(parent) = path.parent() {
            ...
            if let Ok(metadata) = std::fs::metadata(parent) {
                if metadata.permissions().readonly() {
                    tracing::warn!("Service '{}' socket directory may not be writable: '{}'", 
                        service_name, parent.display());
                }
            }
        }
        ...
        if path.exists() {
            tracing::debug!("Service '{}' socket already exists: '{}' (may be from previous run)",
                service_name, socket_path);
        }
        
        tracing::debug!("Service '{}' Unix socket path validated: '{}'", service_name, socket_path);
        Ok(())
    }
    ...
    fn validate_tcp_endpoint(&self, service_name: &str, endpoint: &str) -> Result<(), SinkError> {
        ...
        if !address.contains(':') {
            return Err(SinkError::invalid_config(
                format!("Service '{}' TCP endpoint missing port: '{}'. Use format: tcp://host:port",
                    service_name, endpoint)
            ));
        }
        ...
        if let Err(_) = address.parse::<std::net::SocketAddr>() {
            ...
            if parts.len() != 2 {
                return Err(SinkError::invalid_config(
                    format!("Service '{}' invalid TCP endpoint format: '{}'. Use format: tcp://host:port",
                        service_name, endpoint)
                ));
            }
            ...
            if let Err(_) = parts[1].parse::<u16>() {
                return Err(SinkError::invalid_config(
                    format!("Service '{}' invalid port number in endpoint: '{}'. Port must be 1-65535",
                        service_name, endpoint)
                ));
            }
        }
        
        tracing::debug!("Service '{}' TCP endpoint format validated: '{}'", service_name, endpoint);
        Ok(())
    }
    
    /// Validate WebSocket endpoint format
    fn validate_websocket_endpoint(&self, service_name: &str, endpoint: &str) -> Result<(), SinkError> {
        ...
        tracing::debug!("Service '{}' WebSocket endpoint format validated: '{}'", service_name, endpoint);
        Ok(())
    }
    
    /// Validate composite sink dependencies to prevent runtime failures
    fn validate_composite_dependencies(&self, service_name: &str, config: &ServiceConfig) -> Result<(), SinkError> {
        if let Some(targets) = &config.targets {
            for target_name in targets {
                // Check if target exists in registry
                if !self.registry.contains_service(target_name) {
                    return Err(SinkError::invalid_config(
                        format!("Service '{}' references unknown target '{}'. Available services: [{}]",
...
targets = ["circular"]
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("cannot target itself"));
    }
...
endpoint = "invalid://endpoint"
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("invalid endpoint format"));
    }
}
//! ABI-based event decoding for Polygon DEX events
//!
...
use ethabi::{Event, EventParam, ParamType, RawLog, Token, Address};
use web3::types::{H160, H256, Log, U256};
use protocol_v2::tlv::DEXProtocol;
use std::str::FromStr;
...
pub fn detect_dex_protocol(pool_address: &H160, log: &Log) -> DEXProtocol {
    // Check factory addresses or specific patterns
    let addr_bytes = pool_address.as_bytes();
    
    // V3 pools typically have specific bytecode patterns
    if log.topics.len() >= 3 && log.data.0.len() > 100 {
        // V3 swaps have more data (sqrtPriceX96, liquidity, tick)
        DEXProtocol::UniswapV3
    } else if addr_bytes[0] == 0x5C {
...
impl SwapEventDecoder {
    /// Get Uniswap V2 swap event ABI
    fn v2_swap_event() -> Event {
        Event {
            name: "Swap".to_string(),
            inputs: vec![
    ...
    /// Get Uniswap V3 swap event ABI
    fn v3_swap_event() -> Event {
        Event {
            name: "Swap".to_string(),
            inputs: vec![
                EventParam { name: "sender".to_string(), kind: ParamType::Address, indexed: true },
    ...
    pub fn decode_swap_event(log: &Log, protocol: DEXProtocol) -> Result<ValidatedSwap, DecodingError> {
        let raw_log = RawLog {
        ...
        match protocol {
            ...
            _ => Err(DecodingError::UnsupportedProtocol(protocol)),
        }
    }
    
    /// Decode V3 swap event
    fn decode_v3_swap(pool_address: H160, raw_log: RawLog, protocol: DEXProtocol) -> Result<ValidatedSwap, DecodingError> {
        let event = Self::v3_swap_event();
    ...
    fn decode_v2_swap(pool_address: H160, raw_log: RawLog, protocol: DEXProtocol) -> Result<ValidatedSwap, DecodingError> {
        let event = Self::v2_swap_event();
        let decoded = event.parse_log(raw_log)
            .map_err(|e| DecodingError::AbiParsingError(e.to_string()))?;
        
        // Extract all amounts
        let amount0_in = decoded.params.get(1)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount0In".to_string()))?;
            
        let amount1_in = decoded.params.get(2)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount1In".to_string()))?;
            
        let amount0_out = decoded.params.get(3)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount0Out".to_string()))?;
            
        let amount1_out = decoded.params.get(4)
            .and_then(|p| p.value.clone().into_uint())
    ...
    fn safe_u256_to_i64(value: U256) -> Result<i64, DecodingError> {
        if value > U256::from(i64::MAX) {
            // For very large values, we could:
            // 1. Use scientific notation
            // 2. Truncate with warning
            // 3. Use i128 (would require protocol change)
            // For now, we'll truncate to i64::MAX with warning
...
impl MintEventDecoder {
    /// Get V3 mint event ABI
    fn v3_mint_event() -> Event {
        Event {
            name: "Mint".to_string(),
            inputs: vec![
    ...
    /// Get V2 mint event ABI
    fn v2_mint_event() -> Event {
        Event {
            name: "Mint".to_string(),
            inputs: vec![
                EventParam { name: "sender".to_string(), kind: ParamType::Address, indexed: true },
    ...
    pub fn decode_mint_event(log: &Log, protocol: DEXProtocol) -> Result<ValidatedMint, DecodingError> {
        let raw_log = RawLog {
            topics: log.topics.clone(),
            data: log.data.0.clone(),
        };
    ...
    fn decode_v2_mint(pool_address: H160, raw_log: RawLog, protocol: DEXProtocol) -> Result<ValidatedMint, DecodingError> {
        let event = Self::v2_mint_event();
        let decoded = event.parse_log(raw_log)
            .map_err(|e| DecodingError::AbiParsingError(e.to_string()))?;
        
        // Extract sender as liquidity provider
        let sender = decoded.params.get(0)
            .and_then(|p| p.value.clone().into_address())
            .ok_or(DecodingError::MissingField("sender".to_string()))?;
        
        // Extract amounts
        let amount0 = decoded.params.get(1)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount0".to_string()))?;
            
        let amount1 = decoded.params.get(2)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount1".to_string()))?;
...
impl BurnEventDecoder {
    /// Get V3 burn event ABI
    fn v3_burn_event() -> Event {
        Event {
            name: "Burn".to_string(),
            inputs: vec![
                EventParam { name: "owner".to_string(), kind: ParamType::Address, indexed: true },
    ...
    /// Get V2 burn event ABI
    fn v2_burn_event() -> Event {
        Event {
            name: "Burn".to_string(),
            inputs: vec![
                EventParam { name: "sender".to_string(), kind: ParamType::Address, indexed: true },
    ...
    pub fn decode_burn_event(log: &Log, protocol: DEXProtocol) -> Result<ValidatedBurn, DecodingError> {
        let raw_log = RawLog {
            topics: log.topics.clone(),
            data: log.data.0.clone(),
        };
    ...
    fn decode_v3_burn(pool_address: H160, raw_log: RawLog, protocol: DEXProtocol) -> Result<ValidatedBurn, DecodingError> {
        let event = Self::v3_burn_event();
        let decoded = event.parse_log(raw_log)
            .map_err(|e| DecodingError::AbiParsingError(e.to_string()))?;
        
        // Extract owner
        let owner = decoded.params.get(0)
            .and_then(|p| p.value.clone().into_address())
            .ok_or(DecodingError::MissingField("owner".to_string()))?;
        
        // Extract tick range
        let tick_lower = decoded.params.get(1)
            .and_then(|p| p.value.clone().into_int())
            .ok_or(DecodingError::MissingField("tickLower".to_string()))?;
            
        let tick_upper = decoded.params.get(2)
            .and_then(|p| p.value.clone().into_int())
            .ok_or(DecodingError::MissingField("tickUpper".to_string()))?;
        
        // Extract liquidity and amounts
        let liquidity = decoded.params.get(3)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount".to_string()))?;
            
        let amount0 = decoded.params.get(4)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount0".to_string()))?;
            
        let amount1 = decoded.params.get(5)
            .and_then(|p| p.value.clone().into_uint())
            .ok_or(DecodingError::MissingField("amount1".to_string()))?;
    ...
    fn decode_v2_burn(pool_address: H160, raw_log: RawLog, protocol: DEXProtocol) -> Result<ValidatedBurn, DecodingError> {
        let event = Self::v2_burn_event();
        let decoded = event.parse_log(raw_log)
            .map_err(|e| DecodingError::AbiParsingError(e.to_string()))?;
...
mod tests {
    ...
    fn test_safe_u256_to_i64() {
        ...
        assert_eq!(SwapEventDecoder::safe_u256_to_i64(max_safe).unwrap(), i64::MAX);
        
        // Test overflow
        let overflow = U256::from(i64::MAX) + U256::from(1);
        assert_eq!(SwapEventDecoder::safe_u256_to_i64(overflow).unwrap(), i64::MAX);
    }
    ...
    fn test_dex_protocol_detection() {
        ...
        assert_eq!(detect_dex_protocol(&v3_addr, &v3_log), DEXProtocol::UniswapV3);
        assert_eq!(detect_dex_protocol(&v2_addr, &v2_log), DEXProtocol::UniswapV2);
    }
}
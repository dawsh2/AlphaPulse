...
use web3::types::{Log, H160, H256, Bytes};
...
async fn test_pool_discovery_for_unknown_pool() {
    ...
    assert!(result.is_err(), "Unknown pool should fail gracefully without RPC");
    
    // Test that the pool is not cached after failed discovery
    assert!(!cache.is_cached(&unknown_pool), "Failed discovery should not cache pool");
}
...
async fn test_swap_event_processing_flow() {
    ...
    assert!(mock_log.data.0.len() >= 224, "V3 swap should have >= 224 bytes of data");
}
...
mod integration_helpers {
    ...
    pub fn create_v3_swap_log(pool_address: H160) -> Log {
        ...
        let swap_signature = H256::from([
            0xc4, 0x2c, 0x58, 0xc2, 0x5a, 0x7f, 0x8f, 0xc1,
            0x8d, 0x42, 0xb5, 0x8b, 0x8b, 0xce, 0x25, 0x3b,
            0x85, 0x8c, 0x4d, 0x19, 0x8f, 0x7c, 0xa9, 0x9e,
            0x15, 0x1e, 0x8c, 0x6b, 0x30, 0x5a, 0x8d, 0x47,
        ]);

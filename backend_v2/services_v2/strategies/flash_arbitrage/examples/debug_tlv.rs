            ...
            Ok(bytes_read) => {
                ...
                if bytes_read >= 32 {
                    ...
                    if bytes_read >= 32 + payload_size as usize {
                        ...
                        for (i, chunk) in tlv_data.chunks(16).take(4).enumerate() {
                            print!("      {:04x}: ", i * 16);
                            for byte in chunk {
                                print!("{:02x} ", byte);
                            }
                        ...
                        while offset + 2 <= tlv_data.len() {
                            ...
                            if offset + 2 + tlv_length > tlv_data.len() {
                                println!(
                                    "        ⚠️ Incomplete TLV! Need {} bytes, have {}",
                                    tlv_length,
                            ...
                            if tlv_length > 0 {
                                let payload_preview =
                                    &tlv_data[offset + 2..offset + 2 + tlv_length.min(16)];
                        ...
                        if offset < tlv_data.len() {
                            println!(
                                "      ⚠️ {} trailing bytes after TLVs",
                                tlv_data.len() - offset
                            );
                        }
                    } else {
                        println!(
                            "  ⚠️ Incomplete payload: expected {} bytes, got {}",
                            payload_size,
                            bytes_read - 32
                        );
                    }

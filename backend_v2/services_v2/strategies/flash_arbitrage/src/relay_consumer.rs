...
use torq_types::protocol::tlv::system::TraceId;
use torq_types::{
    PoolSwapTLV,
    SourceType,
...
use torq_codec::parse_header_without_checksum;

// Import from shared types library for financial calculations
use torq_types::common::errors::FixedPointError;
use torq_types::common::fixed_point::{PercentageFixedPoint4, UsdFixedPoint8};

use crate::flash_arbitrage::detector::OpportunityDetector;
use crate::flash_arbitrage::signal_output::SignalOutput;
use torq_state_market::{PoolEvent, PoolStateManager, Stateful};
...
impl RelayConsumer {
    ...
    async fn process_pool_sync(&self, payload: &[u8], timestamp_ns: u64) -> Result<()> {
        ...
        use torq_types::tlv::market_data::PoolSyncTLV;

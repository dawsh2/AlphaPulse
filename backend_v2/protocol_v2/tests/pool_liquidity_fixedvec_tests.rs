...
use protocol_v2::tlv::market_data::PoolLiquidityTLV;
use protocol_v2::tlv::dynamic_payload::{FixedVec, MAX_POOL_TOKENS};
...
fn test_bijection_preservation() {
    ...
    for (i, original_reserves) in test_cases.iter().enumerate() {
        ...
        // Validate accessors
        assert_eq!(tlv.len(), original_reserves.len(), "Test case {}: Length mismatch", i);
        assert_eq!(tlv.get_reserves(), original_reserves.as_slice(), "Test case {}: Slice mismatch", i);
        assert_eq!(tlv.is_empty(), original_reserves.is_empty(), "Test case {}: Empty check failed", i);
    }
}
...
fn test_zero_copy_serialization() {
    ...
    assert_eq!(deserialized_tlv.get_reserves(), original_reserves.as_slice());
...
fn test_memory_layout() {
    ...
    println!("📏 FixedVec<u128, {}> size: {} bytes", MAX_POOL_TOKENS, fixed_vec_size);
...
fn test_edge_cases() {
    ...
    assert_eq!(deserialized.to_reserves_vec(), vec![0u128, u128::MAX, 42u128]);
...
fn test_address_integration() {
    ...
    let eth_address = [
        0x1f, 0x98, 0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98,
        0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54
    ];

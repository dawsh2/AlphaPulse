...
use protocol_v2::define_tlv;
use protocol_v2::tlv::market_data::{TradeTLV, QuoteTLV, PoolSwapTLV};
...
fn test_tlv_sizes_are_aligned() {
    ...
    assert_eq!(std::mem::size_of::<TradeTLV>() % 8, 0, "TradeTLV must be 8-byte aligned");
    assert_eq!(std::mem::size_of::<QuoteTLV>() % 8, 0, "QuoteTLV must be 8-byte aligned");
    assert_eq!(std::mem::size_of::<PoolSwapTLV>() % 8, 0, "PoolSwapTLV must be 8-byte aligned");
}
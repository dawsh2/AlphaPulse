...
fn parse_real_swap_event(log: &Log) -> Option<PoolSwapTLV> {
    ...
    Some(PoolSwapTLV {
        ...
        pool_id: PoolInstrumentId::from_pair(VenueId::Polygon, token0, token1),
        token_in,
        token_out,
        amount_in: amount_in.try_into().unwrap(), // Native precision, no scaling!
        amount_out: amount_out.try_into().unwrap(), // Native precision, no scaling!
        amount_in_decimals: in_decimals,          // Preserve decimal info
        amount_out_decimals: out_decimals,        // Preserve decimal info
...
async fn test_real_polygon_swaps() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for (i, log) in logs.iter().take(5).enumerate() {
        ...
        if let Some(swap) = parse_real_swap_event(log) {
            ...
            let deep_equality_passed = swap.venue == recovered.venue
                && swap.pool_id == recovered.pool_id
                && swap.token_in == recovered.token_in
                && swap.token_out == recovered.token_out
    ...
    if !all_swaps.is_empty() {
        ...
        let total_volume: i64 = all_swaps
            .iter()
            .map(|s| s.amount_in.saturating_abs())
        ...
        println!("  Pool ID encodes: {:#x}, {:#x}", tokens[0], tokens[1]);

        let reconstructed = PoolInstrumentId::from_pair(VenueId::Polygon, tokens[0], tokens[1]);
        assert_eq!(swap.pool_id, reconstructed);

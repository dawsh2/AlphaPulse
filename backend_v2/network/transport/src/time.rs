//! Cached Clock - High-Performance System Time for AlphaPulse Transport
//!
//! ## Purpose
//!
...
//! while maintaining excellent timestamp accuracy for network transport operations.
//!
...
//! - **Accuracy**: ±1ms wall time accuracy (excellent for network timing)
...
mod tests {
    ...
    fn test_external_unix_timestamp_dos_protection() {
        ...
        // Test extremely large timestamp that would overflow
        let overflow_ts = parse_external_unix_timestamp_safe(1e20, "TestExchange");
        assert!(overflow_ts > 0); // Should fallback to current time
    }

    #[test]
    fn test_timestamp_precision_consistency() {

...
impl TransportBridge {
    ...
    pub async fn forward_message(
        ...
        
        {
            ...
            if self.config.enable_deduplication {
                ...
                // Update dedup cache
                buffer.dedup_cache.insert(msg_hash, std::time::Instant::now());
                
                // Clean old entries
                buffer.dedup_cache.retain(|_, time| {
                    time.elapsed().as_secs() < self.config.message_ttl_seconds
                });
            }
    ...
    async fn flush_messages(
        ...
        if remaining_count > 0 {
            warn!("Flushing {} remaining messages from bridge", remaining_count);
            Self::process_messages(buffer, config).await;
        }
    }
    ...
    pub fn is_healthy(&self) -> bool {
        self.worker_handle.as_ref().map(|h| !h.is_finished()).unwrap_or(false)
    }
...
mod tests {
    ...
    async fn test_message_forwarding() {
        ...
        // Forward a message
        let result = bridge.forward_message(
            "node1",
            "actor1",
            b"test message",
            Priority::Normal
    ...
    async fn test_deduplication() {
        ...
        bridge.forward_message("node1", "actor1", message, Priority::Normal).await.unwrap();
        bridge.forward_message("node1", "actor1", message, Priority::Normal).await.unwrap();

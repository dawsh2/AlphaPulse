...
use crate::nodes::InterNodeConfig;
use crate::{validation::TopologyValidator, Actor, Node, Result, TopologyError};
...
impl TopologyConfig {
    ...
    pub fn actors_by_type(&self, actor_type: crate::actors::ActorType) -> Vec<(&String, &Actor)> {
        self.actors
            .iter()
    ...
    pub fn deployment_summary(&self) -> DeploymentSummary {
        ...
        for (_, actor) in &self.actors {
            match actor.actor_type {
                ...
                crate::actors::ActorType::Consumer => summary.consumer_count += 1,
            }

...
use alphapulse_types::{TLVMessageBuilder, TLVType, TradeTLV, VenueId, InstrumentId};
...
impl RelayLogic for MarketDataLogic {
    ...
    fn socket_path(&self) -> &'static str {
        "/tmp/alphapulse/test_market_data.sock"
    }
...
fn test_generic_relay_construction() {
    ...
    assert_eq!(relay.logic.socket_path(), "/tmp/alphapulse/test_market_data.sock");
}
...
fn test_message_routing_logic() {
    ...
    // Parse header
    let header = alphapulse_types::protocol::parse_header(&message).unwrap();
    
    // Test routing logic
    assert!(logic.should_forward(&header), "Should forward market data messages");
    ...
    let wrong_header = alphapulse_types::protocol::parse_header(&wrong_message).unwrap();
    assert!(!logic.should_forward(&wrong_header), "Should not forward signal messages");
}
...
fn test_multiple_relay_types() {
    ...
    impl RelayLogic for SignalLogic {
        ...
        fn socket_path(&self) -> &'static str {
            "/tmp/alphapulse/test_signals.sock"
        }
    }
    ...
    impl RelayLogic for ExecutionLogic {
        ...
        fn socket_path(&self) -> &'static str {
            "/tmp/alphapulse/test_execution.sock"
        }
    }

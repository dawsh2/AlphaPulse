...
# Tests new generic relays with full AlphaPulse component stack under production-like conditions
...
# Configuration
SOCKET_DIR="/tmp/alphapulse_production_sim"
...
start_service "Market Data Relay" \
    "ALPHAPULSE_SOCKET_PATH='$MARKET_DATA_SOCKET' RUST_LOG=info cargo run --release -p alphapulse-relays --bin market_data_relay" \
    "$LOG_DIR/market_data_relay.log"

start_service "Signal Relay" \
    "ALPHAPULSE_SOCKET_PATH='$SIGNAL_SOCKET' RUST_LOG=info cargo run --release -p alphapulse-relays --bin signal_relay" \
    "$LOG_DIR/signal_relay.log"

start_service "Execution Relay" \
    "ALPHAPULSE_SOCKET_PATH='$EXECUTION_SOCKET' RUST_LOG=info cargo run --release -p alphapulse-relays --bin execution_relay" \
...
start_service "Dashboard WebSocket Server" \
    "RUST_LOG=info cargo run --release -p alphapulse-dashboard-websocket -- --port $HEALTH_CHECK_PORT" \
...
if cargo check -p alphapulse-strategies >/dev/null 2>&1; then
    start_service "Flash Arbitrage Strategy" \
        "RUST_LOG=info cargo run --release -p alphapulse-strategies --bin flash_arbitrage" \

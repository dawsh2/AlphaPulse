...
# Configuration
SOCKET_DIR="/tmp/alphapulse_rollback_test"
...
cleanup() {
    ...
    # Kill any remaining AlphaPulse processes
    pkill -f "relay" || true
...
perform_emergency_rollback() {
    ...
    ALPHAPULSE_SOCKET_PATH="$MARKET_DATA_SOCKET" \
    RUST_LOG=info cargo run --release -p alphapulse-relays --bin market_data_relay > "$LOG_DIR/market_data_rollback.log" 2>&1 &
    MARKET_DATA_ROLLBACK_PID=$!
    ROLLBACK_PIDS+=($MARKET_DATA_ROLLBACK_PID)
    
    # Signal Relay
    echo "  Starting original signal_relay..."
    ALPHAPULSE_SOCKET_PATH="$SIGNAL_SOCKET" \
    RUST_LOG=info cargo run --release -p alphapulse-relays --bin signal_relay > "$LOG_DIR/signal_rollback.log" 2>&1 &
    SIGNAL_ROLLBACK_PID=$!
    ROLLBACK_PIDS+=($SIGNAL_ROLLBACK_PID)
    
    # Execution Relay
    echo "  Starting original execution_relay..."
    ALPHAPULSE_SOCKET_PATH="$EXECUTION_SOCKET" \
    RUST_LOG=info cargo run --release -p alphapulse-relays --bin execution_relay > "$LOG_DIR/execution_rollback.log" 2>&1 &
...
ALPHAPULSE_SOCKET_PATH="$MARKET_DATA_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin market_data_relay > "$LOG_DIR/market_data_generic.log" 2>&1 &
MARKET_DATA_PID=$!
CURRENT_PIDS+=($MARKET_DATA_PID)

ALPHAPULSE_SOCKET_PATH="$SIGNAL_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin signal_relay > "$LOG_DIR/signal_generic.log" 2>&1 &
SIGNAL_PID=$!
CURRENT_PIDS+=($SIGNAL_PID)

ALPHAPULSE_SOCKET_PATH="$EXECUTION_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin execution_relay > "$LOG_DIR/execution_generic.log" 2>&1 &
EXECUTION_PID=$!
...
RUST_LOG=info cargo run --release -p alphapulse-dashboard-websocket -- --port $HEALTH_CHECK_PORT > "$LOG_DIR/dashboard_generic.log" 2>&1 &
DASHBOARD_PID=$!

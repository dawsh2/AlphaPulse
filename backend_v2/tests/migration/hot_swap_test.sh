...
# Configuration
SOCKET_DIR="/tmp/alphapulse_hotswap_test"
...
RUST_LOG=info cargo run --release -p alphapulse-relays --bin market_data_relay > "$LOG_DIR/market_data_orig.log" 2>&1 &
...
RUST_LOG=info cargo run --release -p alphapulse-relays --bin signal_relay > "$LOG_DIR/signal_orig.log" 2>&1 &
SIGNAL_ORIG_PID=$!
...
RUST_LOG=info cargo run --release -p alphapulse-relays --bin execution_relay > "$LOG_DIR/execution_orig.log" 2>&1 &
EXECUTION_ORIG_PID=$!
...
RUST_LOG=info cargo run --release -p alphapulse-dashboard-websocket -- --port 8080 > "$LOG_DIR/dashboard.log" 2>&1 &
DASHBOARD_PID=$!
...
ALPHAPULSE_SOCKET_PATH="$MARKET_DATA_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin market_data_relay > "$LOG_DIR/market_data_new.log" 2>&1 &
MARKET_DATA_NEW_PID=$!
...
ALPHAPULSE_SOCKET_PATH="$SIGNAL_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin signal_relay > "$LOG_DIR/signal_new.log" 2>&1 &
SIGNAL_NEW_PID=$!
...
ALPHAPULSE_SOCKET_PATH="$EXECUTION_SOCKET" \
RUST_LOG=info cargo run --release -p alphapulse-relays --bin execution_relay > "$LOG_DIR/execution_new.log" 2>&1 &
EXECUTION_NEW_PID=$!

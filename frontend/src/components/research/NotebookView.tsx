...
export const NotebookView: React.FC<NotebookViewProps> = ({
  ...
  const executeAllCells = async () => {
    ...
    // Get only code cells (skip markdown cells)
    const codeCells = notebookCells.filter(cell => cell.type === 'code');
    
    console.log(`Executing ${codeCells.length} code cells sequentially...`);
    
    for (let i = 0; i < codeCells.length; i++) {
      const cell = codeCells[i];
      console.log(`Executing cell ${i + 1}/${codeCells.length}: ${cell.id}`);
      
      try {
        // Execute the cell and wait for completion
        await executeCell(cell.id);
        
        // Add a small delay between cells to prevent overwhelming the kernel
    ...
    console.log('Finished executing all cells');
    setIsRunningAll(false);
  };

  const generateReport = () => {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const reportName = `${notebookName.replace(/[^a-zA-Z0-9]/g, '_')}_Report_${timestamp}`;
    
    // Collect all cell content and outputs
    let reportContent = `# ${notebookName} - Analysis Report\n\n`;
    reportContent += `Generated: ${new Date().toLocaleString()}\n\n`;
    reportContent += `## Executive Summary\n\n`;
    reportContent += `This report contains the complete analysis output from the ${notebookName} notebook.\n\n`;
    reportContent += `---\n\n`;
    
    notebookCells.forEach((cell, index) => {
      if (cell.type === 'markdown') {
        reportContent += `## Section ${index + 1}: Markdown\n\n`;
        reportContent += `${cell.content}\n\n`;
      } else if (cell.type === 'code') {
        reportContent += `## Section ${index + 1}: Code Analysis\n\n`;
        reportContent += `### Code:\n\`\`\`python\n${cell.content}\n\`\`\`\n\n`;
        ...
        if (cell.images && cell.images.length > 0) {
          reportContent += `### Charts Generated: ${cell.images.length} visualization(s)\n\n`;
        }
      }
    ...
    const codeCells = notebookCells.filter(c => c.type === 'code').length;
    const cellsWithOutput = notebookCells.filter(c => c.output).length;
    const totalImages = notebookCells.reduce((sum, c) => sum + (c.images?.length || 0), 0);
    
    reportContent += `## Analysis Statistics\n\n`;
    reportContent += `- Total Cells: ${totalCells}\n`;
    reportContent += `- Code Cells: ${codeCells}\n`;
    reportContent += `- Cells with Output: ${cellsWithOutput}\n`;
    reportContent += `- Charts Generated: ${totalImages}\n`;
    reportContent += `- Report Generated: ${new Date().toLocaleString()}\n\n`;
    
    reportContent += `## Key Insights\n\n`;
    reportContent += `*Review the analysis output above for detailed findings and recommendations.*\n\n`;
    
    reportContent += `---\n\n`;
    reportContent += `*Report generated by AlphaPulse Analysis Platform*\n`;
    ...
    console.log(`Report generated: ${reportName}.md`);
    alert(`Report downloaded: ${reportName}.md`);
  };
          ...
          {setNotebookName ? (
            <>
              ...
              <button
                ...
                title={isRunningAll ? "Running all cells..." : "Run all code cells"}
              >
                {isRunningAll ? (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const response = await fetch('http://localhost:5001/api/notebook/cleanup', {
                      method: 'POST',

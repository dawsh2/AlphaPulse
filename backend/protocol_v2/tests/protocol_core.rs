...
use alphapulse_protocol_v2::{
    MessageHeader, parse_header, MESSAGE_MAGIC as MAGIC_NUMBER, PROTOCOL_VERSION,
...
fn test_message_size_constraint() {
    ...
    for domain in domains {
        ...
        assert!(msg.len() >= 48, 
            "Message for {:?} is only {} bytes, minimum is 48", domain, msg.len());
    }
}
...
fn test_version_field() {
    ...
    let checksum = alphapulse_protocol_v2::validation::calculate_crc32_excluding_checksum(&msg_v2, 28);
...
fn test_sequence_number_field() {
    ...
    let checksum = alphapulse_protocol_v2::validation::calculate_crc32_excluding_checksum(&msg_with_seq, 28);
...
fn test_flags_field_reserved() {
    ...
    // Recalculate checksum
    let checksum = alphapulse_protocol_v2::validation::calculate_crc32_excluding_checksum(&msg_with_flags, 28);
...
fn test_parse_header_performance() {
    ...
    assert!(ns_per_parse < 1000, 
        "Header parsing too slow: {} ns (target: <1000 ns)", ns_per_parse);
}
...
use alphapulse_protocol_v2::{
    MessageHeader, TLVType, RelayDomain, SourceType, InstrumentId,
    tlv::{TLVMessageBuilder, TLVExtension},
    ProtocolError, PROTOCOL_VERSION, MESSAGE_MAGIC as MAGIC_NUMBER,
};
...
impl InstrumentFixtures {
    pub fn btc() -> InstrumentId {
        ...
        InstrumentId::coin(alphapulse_protocol_v2::VenueId::Binance, "BTC")
    }
    
    pub fn usdt() -> InstrumentId {
        // Create a simple USDT instrument
        InstrumentId::coin(alphapulse_protocol_v2::VenueId::Binance, "USDT")
    }
    
    pub fn eth() -> InstrumentId {
        // Create ETH instrument
        InstrumentId::coin(alphapulse_protocol_v2::VenueId::Binance, "ETH")
    }
    ...
    pub fn btc_usdt_pool() -> InstrumentId {
        ...
        let btc = Self::btc();
        let usdt = Self::usdt();
        InstrumentId::pool(alphapulse_protocol_v2::VenueId::UniswapV2, btc, usdt)
    }
    ...
    pub fn triangular_pool() -> InstrumentId {
        ...
        let eth = Self::eth();
        let usdt = Self::usdt();
        InstrumentId::triangular_pool(alphapulse_protocol_v2::VenueId::Balancer, btc, eth, usdt)
    }
}
...
mod tests {
    ...
    fn test_invalid_messages() {
        ...
        assert!(alphapulse_protocol_v2::parse_header(&invalid_magic).is_err());
        
        let invalid_checksum = create_invalid_checksum_message();
        assert!(alphapulse_protocol_v2::parse_header(&invalid_checksum).is_err());
    }

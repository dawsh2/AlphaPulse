...
use alphapulse_protocol_v2::{
    MessageHeader, parse_header,
    RelayDomain, SourceType, TLVType,
    tlv::{TLVMessageBuilder, ParseError},
...
fn test_single_bit_flip_detection() {
    ...
    for offset in critical_offsets {
        if offset < msg.len() {
            ...
            assert_ne!(actual_checksum, calculated_checksum, 
                      "Bit flip at offset {} should invalidate checksum", offset);
            
            // Parse should detect corruption
            match parse_header(&corrupted) {
                Ok(header) => {
                    // Header parsed, but checksum should be wrong
                    assert!(!verify_message_checksum(&corrupted, {let c = header.checksum; c}, 28),
                           "Should detect corruption at offset {}", offset);
                }
...
fn test_market_data_checksum_skip_performance() {
    ...
    assert!(speedup > 1.5, "Fast parsing should be at least 1.5x faster, got {:.1}x", speedup);
}
...
fn test_signal_domain_enforces_checksum() {
    ...
    match parse_header(&corrupted) {
        ...
        Err(ParseError::ChecksumMismatch { expected, calculated }) => {
...
fn test_execution_domain_audit_trail() {
    ...
    let btc = alphapulse_protocol_v2::InstrumentId::coin(alphapulse_protocol_v2::VenueId::Binance, "BTC");
    let usdt = alphapulse_protocol_v2::InstrumentId::coin(alphapulse_protocol_v2::VenueId::Binance, "USDT");
    let instrument = alphapulse_protocol_v2::InstrumentId::pool(alphapulse_protocol_v2::VenueId::UniswapV2, btc, usdt);
    ...
    let calculated_checksum = calculate_crc32_excluding_checksum(&msg, 28);
    ...
    assert_eq!(msg_checksum, embedded_checksum, "Embedded checksum mismatch");
    
    // Verify the checksum is correct
    assert!(verify_message_checksum(&msg, embedded_checksum, 28),
...
fn test_checksum_with_extended_tlv() {
    ...
    let header = parse_header(&msg).unwrap();
    assert!(verify_message_checksum(&msg, {let c = header.checksum; c}, 28),
    ...
    assert_ne!(orig_checksum, corrupted_checksum,
...
fn test_message_age_validation() {
    ...
    let one_hour_ago = (alphapulse_protocol_v2::header::current_timestamp_ns() 
                        - 3_600_000_000_000) as u64; // 1 hour in nanoseconds
    ...
    assert!(age_ns >= 3_600_000_000_000, "Message should be at least 1 hour old");
    
    // Arbitrage signals older than 1 second should be rejected
    let max_signal_age_ns = 1_000_000_000; // 1 second
    assert!(header.is_older_than(max_signal_age_ns),
...
fn test_cross_platform_checksum_compatibility() {
    ...
    assert_eq!(checksum, manual_checksum,
...
fn test_checksum_performance_impact() {
    ...
    for msg in &messages {
        let _ = verify_message_checksum(msg, 
            u32::from_be_bytes(msg[28..32].try_into().unwrap()), 28);
    }
    let with_check_duration = start_with_check.elapsed();
    
    let overhead_percent = ((with_check_duration.as_nanos() as f64 / 
                             no_check_duration.as_nanos() as f64) - 1.0) * 100.0;
    ...
    assert!(overhead_percent < 50.0, 
           "Checksum overhead too high: {:.1}%", overhead_percent);
}
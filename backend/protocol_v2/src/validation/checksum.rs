...
pub fn verify_message_checksum(data: &[u8], expected_checksum: u32, checksum_offset: usize) -> bool {
...
mod tests {
    ...
    fn test_checksum_excluding_field() {
        ...
        let mut message = vec![0x01, 0x02, 0x03, 0x04, // data before checksum
                              0x00, 0x00, 0x00, 0x00, // checksum field (placeholder)
                              0x05, 0x06, 0x07, 0x08]; // data after checksum
    ...
    fn test_embed_checksum() {
        let mut message = vec![0x01, 0x02, 0x03, 0x04, // data
        ...
        // Verify it's correct
        assert!(verify_message_checksum(&message, embedded_checksum, checksum_offset));
    }

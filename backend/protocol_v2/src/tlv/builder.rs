//! TLV Message Builder
//! 
//! Provides a fluent API for building TLV messages with zero-copy serialization
...
impl TLVMessageBuilder {
    ...
    pub fn add_extended_tlv<T: AsBytes>(mut self, tlv_type: TLVType, data: &T) -> Self {
        let bytes = data.as_bytes();
        if bytes.len() > 65535 {
            panic!("Extended TLV payload too large: {} bytes (max 65535)", bytes.len());
        }
...
mod tests {
    ...
    fn test_basic_message_building() {
        ...
        assert_eq!(payload_size, 26); // 2 + 24

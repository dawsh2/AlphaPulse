...
use super::{ParseError, ParseResult, TLVHeader, TLVType};
use std::mem::size_of;
...
impl RelayTLV {
    ...
    pub fn is_valid_for_domain(&self, domain: crate::RelayDomain) -> bool {
        match domain {
            ...
            crate::RelayDomain::Signal => (20..=39).contains(&self.tlv_type),
    ...
    pub fn get_target_domain(&self) -> crate::RelayDomain {
        match self.tlv_type {
            ...
            _ => crate::RelayDomain::MarketData, // Default for system/recovery types
        }
    }
...
mod tests {
    ...
    fn test_relay_tlv_parsing() {
        ...
        let tlv_payload = &message[crate::header::MessageHeader::SIZE..];

...
use crate::tlv::{TLVType, TLVMessageBuilder};
use crate::{RelayDomain, SourceType, ProtocolError};
...
impl SnapshotLoader {
    ...
    pub fn parse_snapshot(payload: &[u8]) -> Result<(SnapshotTLVHeader, SnapshotData), ProtocolError> {
        if payload.len() < std::mem::size_of::<SnapshotTLVHeader>() {
            return Err(ProtocolError::Recovery("Snapshot payload too small".to_string()));
        }
        ...
        let decompressed_data = match CompressionType::from_u8(header.compression_type) {
            ...
            _ => {
                return Err(ProtocolError::Recovery(
                    format!("Unsupported compression type: {}", header.compression_type)
                ));
            }
        };
        ...
        if calculated_checksum != expected_checksum {
            return Err(ProtocolError::Recovery(
                format!("Snapshot checksum mismatch: expected {:#x}, got {:#x}", 
        ...
        if decompressed_data.len() != expected_size {
            return Err(ProtocolError::Recovery(
                format!("Snapshot size mismatch: expected {}, got {}", 
                       expected_size, decompressed_data.len())
            ));
        }
    ...
    pub fn apply_snapshot(snapshot_data: SnapshotData) -> Result<(u64, RelayState), ProtocolError> {
        ...
        if snapshot_data.version != 1 {
            return Err(ProtocolError::Recovery(
                format!("Unsupported snapshot version: {}", snapshot_data.version)
            ));
        }
        ...
        if !snapshot_data.is_fresh(max_age) {
            return Err(ProtocolError::Recovery(
                format!("Snapshot too old: {} seconds", snapshot_data.age_ns() / 1_000_000_000)
            ));
        }
...
mod tests {
    use super::*;
    use crate::tlv::parser::{parse_header, find_tlv_by_type};
    ...
    fn create_test_relay_state() -> RelayState {
        ...
        active_instruments.insert(12345, InstrumentInfo {
            instrument_id: 12345,
    ...
    fn test_snapshot_data_serialization() {
        ...
        assert_eq!(snapshot.state.consumer_sequences, deserialized.state.consumer_sequences);
        assert_eq!(snapshot.state.message_buffer.len(), deserialized.state.message_buffer.len());
    }
    ...
    fn test_snapshot_builder() {
        ...
        let message = builder.build_snapshot(
            RelayDomain::Signal,
    ...
    fn test_snapshot_application() {
        ...
        assert_eq!(restored_state.message_buffer.len(), state.message_buffer.len());
        // Check active instruments match
        assert_eq!(restored_state.active_instruments.len(), state.active_instruments.len());
    ...
    fn test_checksum_validation() {
        ...
        let mut message = builder.build_snapshot(
        ...
        let header = parse_header(&message).unwrap();
        let tlv_payload = &message[32..];
        let mut snapshot_payload = find_tlv_by_type(tlv_payload, TLVType::Snapshot as u8).unwrap().to_vec();
        
        // Now corrupt just the snapshot payload data (after its own header)
        // This won't affect the message header checksum since we're working with extracted data
        if snapshot_payload.len() > 20 {
            snapshot_payload[20] ^= 0xFF; // Corrupt snapshot data, not its header
        }
        ...
        assert!(result.is_err(), "Should fail due to corrupted snapshot data");
    }
}
...
use alphapulse_protocol_v2::{
    ...
    tlv::{TLVMessageBuilder, parse_tlv_extensions_for_relay},
    MessageHeader,
};
...
fn test_33_byte_payload_parsing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_tlv_extensions_for_relay(tlv_payload) {
        Ok(tlvs) => {
            info!("   ✅ SUCCESS: Parsed {} TLVs with optimized parser", tlvs.len());
            info!("   TLV Type: {}, Length: {}", tlvs[0].tlv_type, tlvs[0].tlv_length);
...
async fn test_optimized_relay_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("      Messages processed: {}/{} ({:.1}%)", processed_count, BATCH_SIZE, 
    ...
    if processed_count == BATCH_SIZE {
        ...
        if throughput > 1_000_000.0 {
            info!("   🚀 EXCEEDS 1M MSG/S TARGET: {:.1}x faster than target", throughput / 1_000_000.0);
        } else {
            info!("   📊 Throughput: {:.1}% of 1M msg/s target", (throughput / 1_000_000.0) * 100.0);
        }
    } else {
...
async fn test_parser_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bad_message = TLVMessageBuilder::new(
        RelayDomain::MarketData,
        SourceType::BinanceCollector
    )
    ...
    match alphapulse_protocol_v2::parse_tlv_extensions(bad_tlv_payload) {
    ...
    match parse_tlv_extensions_for_relay(bad_tlv_payload) {
        Ok(tlvs) => info!("      ✅ Optimized parser succeeds with {} TLVs", tlvs.len()),

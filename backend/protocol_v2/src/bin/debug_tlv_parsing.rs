...
use alphapulse_protocol_v2::{
    ...
    parse_tlv_extensions, MessageHeader,
};
...
fn test_basic_tlv_parsing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("   Creating TLV message with {} byte payload...", trade_payload.len());
    ...
    info!("   TLV bytes: {:?}", &tlv_payload[..std::cmp::min(20, tlv_payload.len())]);
...
fn test_performance_test_scenario() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..BATCH_SIZE {
        ...
        if i % 100 == 0 {
            info!("   Processed {}/{}, successful: {}", i, BATCH_SIZE, processed_count);
        }
    }
    
    info!("   Final result: {}/{} messages processed successfully", processed_count, BATCH_SIZE);
    ...
    if processed_count == 0 {
        ...
        warn!("   ⚠️  Partial success: {:.1}% messages processed", (processed_count as f64 / BATCH_SIZE as f64) * 100.0);
    }
...
fn test_tlv_validation_overhead() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_cases = vec![
        ...
        ("Invalid Trade (20 bytes)", TLVType::Trade, vec![0; 20], false),
        ("Invalid Trade (30 bytes)", TLVType::Trade, vec![0; 30], false),

...
/// QuickSwap-specific pool implementation
...
impl QuickSwapPool {
    ...
    async fn query_tokens(&self) -> Result<(String, String)> {
        ...
        if let (Some(token0_hex), Some(token1_hex)) = (
            ...
            debug!("QuickSwap pool {} tokens: token0={}, token1={}", 
                self.address, token0_addr, token1_addr);
            
            Ok((token0_addr, token1_addr))
        } else {
            Err(anyhow::anyhow!("Failed to query QuickSwap pool tokens"))
        }
    }
}

#[async_trait]
impl DexPool for QuickSwapPool {
    ...
    fn parse_swap_event(&self, data: &str) -> Result<SwapEvent> {
        // QuickSwap uses UniswapV2 swap event format
        let hex_data = data.strip_prefix("0x").unwrap_or(data);
        
        if hex_data.len() < 256 {
            return Err(anyhow::anyhow!("Invalid QuickSwap swap event data length"));
        }
    ...
    fn calculate_price(&self, swap: &SwapEvent) -> Price {
        ...
        if price == 0.0 {
            warn!("QuickSwap: Complex swap with multiple ins/outs, using simple ratio");
        }
    ...
    fn format_symbol(&self, token0: &str, token1: &str) -> String {
        // QuickSwap-specific symbol formatting
        format!("QS:{}/{}", token0, token1)
    }
}